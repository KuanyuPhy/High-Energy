...
*/
b_tpl = BASF_Histogram->ntuple("B0 --> hh","gas evtcount chisqexk mbc de ebeam pxb pyb pzb eb ptb chix chiy chiz chie chim gax gay gaz gae gam cos1 cos1cm cosh1 gax1 gay1 gaz1 gae1 gam1 pi0v pi0p pi0v1 pi0p1 e9oe25 e9oe251 etapx etapy etapz etae etam ipx ipy ipz bvertexx bvertexy bvertexz overtexx overtexy overtexz exfit pmiss emiss vtflag deltaz R2 costhr costhp spher cosb R2s R1so R2so R3so R4so R1gso R2gso R3gso R4gso R1oo R2oo R3oo R4oo EVTID RUNID EXPID eventid farmid imm2 mmm2 et H0oo H1oo H2oo H3oo H4oo H0son H1son H2son H3son H4son H0soc H1soc H2soc H3soc H4soc H0som H1som H2som H3som H4som qr flavor imode hi1 hi2 hi3 hi4 hi5 hi6 mo1 gmo1 mo2 gmo2 mo3 gmo3 mo4 gmo4 mo5 gmo5 mo6 gmo6 bnd1 bnd2 bnd3 bnd4 bnd5 bnd6 bnd7 bnd8 bnd9 nbnd charged enerbpd1 enerbpd2 enerbpd3 enerbpd4 enerbpd5 enerbpd6 enerbpd7 enerbpd8 enerbpd9 hindex hrepeat bpd1 bpd2 bpd3 bpd4 bpd5 nbpd");
#endif
...
*/
	...
	//std::cout << "start combination" << endl;
	...
	for (std::vector<Particle>::iterator i = B_cand1.begin(); i != B_cand1.end(); i++)
	{
		...
		//le of eta
		HepLorentzVector b_p0((*i).child(1).p());
		...
		Particle &ch1 = (*i).child(1).child(0);
		Particle &ch2 = (*i).child(1).child(1);
		...
		// Vertex Constrain
		//=========================================
		Mdst_gamma Mdst_ch1 = ch1.mdstGamma();
		Mdst_gamma Mdst_ch2 = ch2.mdstGamma();
		...
		ExKFitterParticle KF_ch1(Mdst_ch1, IP, IPerr);
		ExKFitterParticle KF_ch2(Mdst_ch2, IP, IPerr);
		...
		//eta.LinkParticle(&KF_ch1);
		//eta.LinkParticle(&KF_ch2);
		//eta.LinkMass(&eta_Mass);
		//eta.LinkVertex(&eta_Vertex);
		
		//ExKFitterParticle con1;
                //con1.LinkParticle(&KF_ch1);
                //con1.LinkParticle(&KF_ch2);
		...
		if (MCstatus == 1)
		{
			...
			const Mdst_charged ch3 = (*i).child(0).child(0).mdstCharged(); //chic01pi+
			const Mdst_charged ch4 = (*i).child(0).child(1).mdstCharged(); //chic01pi-
			const Mdst_charged ch5 = (*i).child(0).child(2).mdstCharged(); //chic02pi+2
			const Mdst_charged ch6 = (*i).child(0).child(3).mdstCharged(); //chic02pi-2
			...
			if (Evtch1.idhep() == 22 && Evtch2.idhep() == 22 && Evtch3.idhep() == 211 && Evtch4.idhep() == 211 && Evtch5.idhep() == -211 && Evtch6.idhep() == -211)
			{
				if (EvtP1.idhep() == 221 && EvtP2.idhep() == 221 && EvtP3.idhep() == 10441 && EvtP4.idhep() == 10441 && EvtP5.idhep() == 10441 && EvtP6.idhep() == 10441 &&
					abs(EvtGP3.idhep()) == 511 && abs(EvtGP4.idhep()) == 511 && abs(EvtGP5.idhep()) == 511 && abs(EvtGP6.idhep()) == 511 && abs(EvtGP1.idhep()) == 511 && abs(EvtGP2.idhep()) == 511)
				{
