...
// FILE : chi_c0_eta_pipipipi_pipipi.cc
// Author : Tzuying
...
///////////問學長這裡需要改class的名稱嗎？
// Module class
class chi_c0_eta_pipipipi_pipipi : public Module
{ //start class chi_c0_eta_pipipipi_pipipi
public:
	chi_c0_eta_pipipipi_pipipi(void){};
	~chi_c0_eta_pipipipi_pipipi(void){};
	void init(int *);
	void term(void);
...
////////////////要不要改class
void chi_c0_eta_pipipipi_pipipi::init(int *)
{ //start void chi_c0_eta_pipipipi_pipipi::init(int*)
	evtcount = 0;
...
} //end void chi_c0_eta_pipipipi_pipipi::init(int*)

////////////////////////////////////設這個物件要幹嘛呢？把他//會不會怎麼樣？////////////////////////////////////
void chi_c0_eta_pipipipi_pipipi::term(void)
{   //start void chi_c0_eta_pipipipi_pipipi::term(void)
	/*
		...
		*/
} //end void chi_c0_eta_pipipipi_pipipi::term(void)

// Set IP location   ////////////// HepPoint3D是class嗎？為什麼不用::////////////////////////
HepPoint3D IP(0, 0, 0);
HepSymMatrix IPerr(3, 0);

void chi_c0_eta_pipipipi_pipipi::begin_run(BelleEvent *evptr, int *status)
{					  //start void chi_c0_eta_pipipipi_pipipi::begin_run
...
} //end void chi_c0_eta_pipipipi_pipipi::begin_run

///////////////////////////重點開始！////////
///////////////////////////////////一開始的class需要換嗎？//////////////////////////
void chi_c0_eta_pipipipi_pipipi::event(BelleEvent *evptr, int *status) //～～～～～～～～為什麼逗號前面的不用宣告呢～～～～～～～～～～
{																	   //start void chi_c0_eta_pipipipi_pipipi::event(BelleEvent *evptr, int *status)
...
*/
	...
	for (std::vector<Particle>::iterator i = pi_plus.begin(); i != pi_plus.end(); i++)
	{
		...
		if (abs(dr) < alldr && abs(dz) < alldz)
		{
			...
			b_tpl->column("pidr", dr);
			b_tpl->column("pidz", dz);
		}
	}
	for (std::vector<Particle>::iterator i = pi_minus.begin(); i != pi_minus.end(); i++)
	{
		...
		if (abs(dr) < alldr && abs(dz) < alldz)
		{
			...
			b_tpl->column("pi1dr", dr);
			b_tpl->column("pi1dz", dz);
		}
	}
	...
	for (std::vector<Mdst_pi0>::iterator i = pi0_mag.begin(); i != pi0_mag.end(); i++)
	{
		...
		//end Gamma selection //772
		//		   b_tpl->dumpData();
		//                   *status = 1;
	} //end pi_0 //757
	//std::cout << "end pi0 selection " << endl;
	//****
	for (std::vector<Particle>::iterator i2 = gamma1.begin(); i2 != gamma1.end(); i2++)
	{
		...
		{
			//cout << "start pi0 begin" << endl;

			//cout << "start gamma begin" << endl;
			for (int k = 0; k < 2; k++)
			{
		...
		for (std::vector<Particle>::iterator i2 = i + 1; i2 != tpi_plus.end(); i2++)
		{
			...
			for (std::vector<Particle>::iterator i3 = tpi_minus.begin(); i3 != tpi_minus.end(); i3++)
			{
				...
				for (std::vector<Particle>::iterator i4 = i3 + 1; i4 != tpi_minus.end(); i4++)
				{
					...
					if (particle_chic_pipi.p().mag() > 3 && particle_chic_pipi.p().mag() < 4)
					{
	...
	//std::cout << "done all particle selection" << endl;
	...
	for (std::vector<Particle>::iterator i = B_cand1.begin(); i != B_cand1.end(); i++)
	{
		...
		if ((*i).child(0).child(0).mdstCharged().get_ID() == (*i).child(0).child(1).mdstCharged().get_ID() || (*i).child(0).child(0).mdstCharged().get_ID() == (*i).child(1).child(0).mdstCharged().get_ID())
		{
			continue;
		}
		//fill ntuple of Xc0_2pi+
		b_tpl->column("p1px", (*i).child(0).child(1).px());
		b_tpl->column("p1py", (*i).child(0).child(1).py());
		b_tpl->column("p1pz", (*i).child(0).child(1).pz());
		b_tpl->column("p1e", (*i).child(0).child(1).e());
		b_tpl->column("p1m", (*i).child(0).child(1).p().mag());
		//fill ntuple of eta_pi+
		b_tpl->column("p2px", (*i).child(1).child(0).px());
		b_tpl->column("p2py", (*i).child(1).child(0).py());
		b_tpl->column("p2pz", (*i).child(1).child(0).pz());
		b_tpl->column("p2e", (*i).child(1).child(0).e());
		b_tpl->column("p2m", (*i).child(1).child(0).p().mag());
		//fill ntuple of Xc0_1pi-
		b_tpl->column("ppxm", (*i).child(0).child(2).px());
		b_tpl->column("ppym", (*i).child(0).child(2).py());
		b_tpl->column("ppzm", (*i).child(0).child(2).pz());
		b_tpl->column("pem", (*i).child(0).child(2).e());
		b_tpl->column("pmm", (*i).child(0).child(2).p().mag());
		if ((*i).child(0).child(2).mdstCharged().get_ID() == (*i).child(0).child(3).mdstCharged().get_ID() || (*i).child(0).child(2).mdstCharged().get_ID() == (*i).child(1).child(1).mdstCharged().get_ID())
		{
			continue;
		}
		//fill ntuple of Xc0_2pi-
		b_tpl->column("p1pxm", (*i).child(0).child(3).px());
		b_tpl->column("p1pym", (*i).child(0).child(3).py());
		b_tpl->column("p1pzm", (*i).child(0).child(3).pz());
		b_tpl->column("p1em", (*i).child(0).child(3).e());
		b_tpl->column("p1mm", (*i).child(0).child(3).p().mag());
		//fill ntuple of eta_2pi-
		b_tpl->column("p2pxm", (*i).child(1).child(1).px());
		b_tpl->column("p2pym", (*i).child(1).child(1).py());
		b_tpl->column("p2pzm", (*i).child(1).child(1).pz());
		b_tpl->column("p2em", (*i).child(1).child(1).e());
		b_tpl->column("p2mm", (*i).child(1).child(1).p().mag());
		//eta pi0
		b_tpl->column("pi0px", (*i).child(1).child(2).px());
		b_tpl->column("pi0py", (*i).child(1).child(2).py());
		b_tpl->column("pi0pz", (*i).child(1).child(2).pz());
		...
		//if ((*i).child(1).child(2).child(0).mdstCharged().get_ID() == (*i).child(1).child(2).child(1).mdstCharged().get_ID())
		//{
		//	continue;
		//}
		...
		kvertexfitter kvf;
		addTrack2fit(kvf, (*i).child(0).child(0)); // add "pi+" to kfitter chic01
		addTrack2fit(kvf, (*i).child(0).child(2)); // add "pi-" to kfitter chic01
		addTrack2fit(kvf, (*i).child(0).child(1)); // add "pi+" to kfitter chic02
		addTrack2fit(kvf, (*i).child(0).child(3)); // add "pi-" to kfitter chic02
		...
		//------------------- end PiPi vertex fit with K fitter  -------------------------
		//------------------- eta 's pi kpi probability   -------------------------
		const Mdst_charged *ch10 = &(*i).child(1).child(0).mdstCharged();
		float pi10_p = selkpi.prob(ch10);
		const Mdst_charged *ch11 = &(*i).child(1).child(1).mdstCharged();
		float pi11_p = selkpi.prob(ch11);
		double pi0mass = (*i).child(1).child(2).p().mag();
		//if (pi10_p < 0.4 && pi11_p < 0.4)
		//{
		//	continue;
		//}
		b_tpl->column("pi10_p", selkpi.prob(ch10));
		b_tpl->column("pi11_p", selkpi.prob(ch11));
		...
		*/
		...
		Particle &ch3 = (*i).child(0).child(2);			 //pi-
		Particle &ch4 = (*i).child(0).child(3);			 //pi-
		Particle &ch5 = (*i).child(1).child(0);			 //etapi+
		Particle &ch6 = (*i).child(1).child(1);			 //etapi-
		Particle &ch7 = (*i).child(1).child(2).child(0); //pi0->gamma1
		Particle &ch8 = (*i).child(1).child(2).child(1); //pi0->gamma2
		...
		//ExKFitterMass Pi_Mass(0.134976);
		...
		//------------------------- MC truth matching --------------------------------------------------------------------------------------------------
		//cout << "start to MC truth matching " << endl;
		// the record of HEP_ID (only for Sig_MC)_from ywdoung
		if (MCstatus == 1)
		{
			const Mdst_charged ch1 = (*i).child(0).child(0).mdstCharged();		//chic01pi+
			const Mdst_charged ch2 = (*i).child(0).child(1).mdstCharged();		//chic01pi+2
			const Mdst_charged ch3 = (*i).child(0).child(2).mdstCharged();		//chic02pi-
			const Mdst_charged ch4 = (*i).child(0).child(3).mdstCharged();		//chic02pi-2
			...
			if (Evtch1.idhep() == 211 && Evtch2.idhep() == 211 && Evtch3.idhep() == -211 && Evtch4.idhep() == -211 && Evtch5.idhep() == 211 && Evtch6.idhep() == -211 && Evtch7.idhep() == 22 && Evtch8.idhep() == 22)
			{
				if (EvtP1.idhep() == 10441 && EvtP2.idhep() == 10441 && EvtP3.idhep() == 10441 && EvtP4.idhep() == 10441 && EvtP5.idhep() == 221 && EvtP6.idhep() == 221 &&
					EvtP7.idhep() == 111 && EvtP8.idhep() == 111 && abs(EvtGP1.idhep()) == 511 && abs(EvtGP2.idhep()) == 511 && abs(EvtGP3.idhep()) == 511 && abs(EvtGP4.idhep()) == 511 && abs(EvtGP5.idhep()) == 511 && abs(EvtGP6.idhep()) == 511 && abs(EvtGGP7.idhep()) == 511 && abs(EvtGGP8.idhep()) == 511 && EvtGP7.idhep() == 221 && EvtGP8.idhep() == 221)
				{
					if (Evtch1.mo(0) == Evtch2.mo(0) && Evtch1.mo(0) == Evtch3.mo(0) && Evtch1.mo(0) == Evtch4.mo(0))
					{
						if (Evtch5.mo(0) == Evtch6.mo(0) && Evtch5.mo(0) == EvtP7.mo(0) && Evtch5.mo(0) == EvtP8.mo(0))
						{
							if (Evtch7.mo(0) == Evtch8.mo(0))
							{
								if (EvtP1.mo(0) == EvtP2.mo(0) && EvtP1.mo(0) == EvtP3.mo(0) && EvtP1.mo(0) == EvtP4.mo(0) && EvtP1.mo(0) == EvtP5.mo(0) && EvtP1.mo(0) == EvtP6.mo(0) && EvtP1.mo(0) == EvtGP7.mo(0) && EvtP1.mo(0) == EvtGP8.mo(0))
								{
									hindex = 1;
								}
							}
...
//void chi_c0_eta_pipipipi_pipipi::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P)
void chi_c0_eta_pipipipi_pipipi::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P, HepPoint3D &bvertex, int &vertexflag, HepPoint3D &overtex)
{
...
void chi_c0_eta_pipipipi_pipipi::GetImpactParameters(const Mdst_charged *charged, double *dr, double *dz, int t)
{
