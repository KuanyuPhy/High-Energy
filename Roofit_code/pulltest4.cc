...
int main(int argc, char *argv[])
{
	...
	// ----- 1-dim deltaE PDF -----
        RooRealVar mean("deltae_signal_mean", "de signal mean",-0.000855);
        RooRealVar sigma_delta1("sigma_delta1","sigma_delta1",0.01347);//0.04,0,0.3
        RooRealVar alpha1("alpha1","alpha1",1.035);
        RooRealVar n1("n1","n1",3.38);
        RooCBShape sig1("sig1", "Signal component 1",de,mean,sigma_delta1,alpha1,n1);
        RooRealVar mga1_mean("mga1_mean", "mga1_mean",-0.06025);
        RooRealVar mga1_width("mga1_width", "mga1_width",0.1);
        RooGaussian sig2("sig2", "sig2",de, mga1_mean, mga1_width);

	RooRealVar sig1frac("sig1frac", "fraction of component 1 in signal",0.686);
        RooAddPdf deltae_signal("deltae_signal", "de signal_pdf", RooArgList(sig1,sig2), sig1frac);	
    
    // -------- bb de background ------------
    
        RooRealVar deltae_a0("deltae_a0", "deltae_a0", -0.7224);
        RooRealVar deltae_a1("deltae_a1", "deltae_a1", 0.155);
    ...
    // --------- qq de background ------------
	RooRealVar deltae_a2("deltae_a2", "deltae_a2",-0.4596);
        RooRealVar deltae_a3("deltae_a3", "deltae_a3", 0.036);
        //RooRealVar deltae_a2("deltae_a1", "deltae_a1", 0.1, -1.0, 1.0);
...
RooRealVar N_qq("N_qq", "N_qq",983,-1000.0, 20000.0);
 ...
 float toynumsig=a.PoissonD(0.7218),toynumbb=b.PoissonD(21662),toynumqq=c.PoissonD(3448);// my expect value out put with poisson distribution
 //75464 23032
