...
// Module class
class ana_d0_20141217 : public Module 
{
public:
  ana_d0_20141217(void){};
  ~ana_d0_20141217(void){};
  void init(int*);
  void term(void);
...
extern "C" Module_descr *mdcl_ana_d0_20141217()
{
  ana_d0_20141217 *module = new ana_d0_20141217;
  Module_descr *dscr = new Module_descr ( "ana_d0_20141217", module );
...
//ntuple name length--> 8 characters(upper limit)
  b_tpl = BASF_Histogram->ntuple("B0 --> D0 pi","massd0 pxd0 pyd0 pzd0 ed0 mass0 px0 py0 pz0 e0 pid0 chrg0 dr0 dz0 cos0 mass1 px1 py1 pz1 e1 pid1 chrg1 dr1 dz1 cos1 vchisq chisqexk hindex hi0 mo0 gmo0 hi1 mo1 gmo1");
...
void ana_d0_20141217::term (void)
{
/*
...
void ana_d0_20141217::begin_run(BelleEvent *evptr, int *status)
{
...
void 
ana_d0_20141217::event(BelleEvent *evptr, int *status)
{ 
...
//------------------- D0 vertex fit with ExKfitter  -------------------------
        ...
        D0_init.setX(IP.x() + (*i).p().px()/(*i).p().rho());
        D0_init.setY(IP.y() + (*i).p().py()/(*i).p().rho());
        D0_init.setZ(IP.z() + (*i).p().pz()/(*i).p().rho());
        ExKFitterVertex D0_Vertex(D0_init);

        ExKFitterParticle D0;
        D0.LinkParticle(&KF_0);
        D0.LinkParticle(&KF_1);
...
if (MCstatus == 1)
{
	...
	b_tpl->column("hi1", Evtch_1.idhep());
        ...
        if(Evtch_0.idhep() == -321 && Evtch_1.idhep() == 211 && EvtP_0.idhep() == 421 && EvtP_1.idhep() == 421)
        { hindex = 1;}
        else if (Evtch_0.idhep() == 321 && Evtch_1.idhep() == -211 && EvtP_0.idhep() == -421 && EvtP_1.idhep() == -421 )
        { hindex = 1;}
        else
        { hindex = 0;}
...
//void ana_d0_20141217::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P)
void ana_d0_20141217::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13],Vector4 &otherB_P, HepPoint3D &bvertex, int & vertexflag, HepPoint3D & overtex)
{
...
void ana_d0_20141217::GetImpactParameters(const Mdst_charged *charged, double *dr, double *dz, int t) 
     {
