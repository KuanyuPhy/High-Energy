...
// Module class
class ana_birdegg : public Module 
{
public:
  ana_birdegg(void){};
  ~ana_birdegg(void){};
...
extern "C" Module_descr *mdcl_ana_birdegg()
{
  ana_birdegg *module = new ana_birdegg;
  Module_descr *dscr = new Module_descr ( "ana_birdegg", module );
...
//branch name length--> 8 characters(upper limit)
  b_tpl = BASF_Histogram->ntuple("BirdEgg","ebeam EVTID RUNID EXPID eventid farmid \
  nCh nPi nK nP \  
  ");
...
void ana_birdegg::begin_run(BelleEvent *evptr, int *status)
{
...
void 
ana_birdegg::event(BelleEvent *evptr, int *status)
{ 
...
*/
      ...
      b_tpl->column("nCh", nCh);
      b_tpl->column("nP", nP);
      b_tpl->column("nPi", nPi);
      b_tpl->column("nK", nK);
...
//void ana_birdegg::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P)
void ana_birdegg::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13],Vector4 &otherB_P, HepPoint3D &bvertex, int & vertexflag, HepPoint3D & overtex)
{
...
void ana_birdegg::GetImpactParameters(const Mdst_charged *charged, double *dr, double *dz, int t) 
     {
