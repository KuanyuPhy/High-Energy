...
// Module class
class ana_example : public Module 
{
public:
  ana_example(void){};
  ~ana_example(void){};
...
extern "C" Module_descr *mdcl_ana_example()
{
  ana_example *module = new ana_example;
  Module_descr *dscr = new Module_descr ( "ana_example", module );
...
//ntuple name length--> 8 characters(upper limit)
  b_tpl = BASF_Histogram->ntuple("B0 --> D0 pi","massd0 px0 px1");
#endif
...
void ana_example::begin_run(BelleEvent *evptr, int *status)
{
...
void 
ana_example::event(BelleEvent *evptr, int *status)
{ 
...
//    combination(D0,ptype_D0,k_minus,pi_plus);
	combination(jpsi,ptype_jpsi,e_minus,e_plus);


//for B candicate 2:

	std::count << "--------Evt--------" << std::endl;


  for(std::vector<Particle>::iterator i =jpsi.begin(); i != jpsi.end(); i++)
  { 

#ifdef BTUPLE

	std::cout << (*i).p().mag() << std::endl;
...
//void ana_example::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P)
void ana_example::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13],Vector4 &otherB_P, HepPoint3D &bvertex, int & vertexflag, HepPoint3D & overtex)
{
...
void ana_example::GetImpactParameters(const Mdst_charged *charged, double *dr, double *dz, int t) 
     {
