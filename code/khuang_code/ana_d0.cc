...
extern "C" Module_descr *mdcl_ana_d0()
{
  ana_d0 *module = new ana_d0;
  Module_descr *dscr = new Module_descr ( "ana_d0", module );
...
void ana_d0::begin_run(BelleEvent *evptr, int *status)
{
...
void 
ana_d0::event(BelleEvent *evptr, int *status)
{ 
...
//void ana_d0::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P)
void ana_d0::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13],Vector4 &otherB_P, HepPoint3D &bvertex, int & vertexflag, HepPoint3D & overtex)
{
...
void ana_d0::GetImpactParameters(const Mdst_charged *charged, double *dr, double *dz, int t) 
     {
