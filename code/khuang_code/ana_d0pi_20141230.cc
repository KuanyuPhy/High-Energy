...
// Module class
class ana_d0pi_20141230 : public Module 
{
public:
  ana_d0pi_20141230(void){};
  ~ana_d0pi_20141230(void){};
  void init(int*);
  void term(void);
...
extern "C" Module_descr *mdcl_ana_d0pi_20141230()
{
  ana_d0pi_20141230 *module = new ana_d0pi_20141230;
  Module_descr *dscr = new Module_descr ( "ana_d0pi_20141230", module );
...
void 
ana_d0pi_20141230::hist_def(void)
{ 
...
//ntuple name length--> 8 characters(upper limit)
  b_tpl = BASF_Histogram->ntuple("B0 --> D0 pi","de mbc massb pxb pyb pzb eb \
  massd0 pxd0 pyd0 pzd0 ed0 \
  mass00 px00 py00 pz00 e00 pid00 chrg00 dr00 dz00 cos00 \
  mass01 px01 py01 pz01 e01 pid01 chrg01 dr01 dz01 cos01 \
  mass1 px1 py1 pz1 e1 pid1 chrg1 dr1 dz1 cos1 \
  vchisq chisqexk hindex hi00 mo00 gmo00 hi01 mo01 gmo01 hi01 mo1 gmo1 \
...
void ana_d0pi_20141230::init(int*)
{
  evtcount=0;
...
void ana_d0pi_20141230::term (void)
{
/*
...
void ana_d0pi_20141230::begin_run(BelleEvent *evptr, int *status)
{
...
void 
ana_d0pi_20141230::event(BelleEvent *evptr, int *status)
{ 
...
/*
    combination(Sigma0_list,ptype_Sigma0,Lam_list,gamma);
    combination(Sigma0_bar_list,ptype_Sigma0bar,Lambar_list,gamma);
    combination(B_cand1,ptype_Bminus,p_plus,Sigma0_bar_list,gamma);
    combination(B_cand1,ptype_Bplus,p_minus,Sigma0_list,gamma);
*/

    combination(D0,ptype_D0,k_minus,pi_plus);
    combination(D0B,ptype_D0B,k_plus,pi_minus);
    combination(B_cand1,ptype_Bminus,D0,pi_minus);
    combination(B_cand1,ptype_Bplus,D0B,pi_plus);
...
//------------------- information of D0 (*i).child(0) -------------------------

	b_tpl->column("massd0", (*i).child(0).p().mag());
	b_tpl->column("pxd0", (*i).child(0).px());
	b_tpl->column("pyd0", (*i).child(0).py());
	b_tpl->column("pzd0", (*i).child(0).pz());
	b_tpl->column("ed0", (*i).child(0).e());



//------------------- information of K from D0 (*i).child(0).child(0) -------------------------
        ...
        const Mdst_charged* chk00 = &(*i).child(0).child(0).mdstCharged();
        float kpi_id00 = selkpi.prob(chk00);
        b_tpl->column("pid00", kpi_id00);
          
        float chrg00 = chk00->charge();
        b_tpl->column("chrg00", chrg00);

        double dr00,dz00;
        GetImpactParameters(chk00,&dr00,&dz00,3);
        b_tpl->column("dr00", float(dr00));
        b_tpl->column("dz00", float(dz00));
...
//------------------- information of pi from D0 (*i).child(0).child(1) -------------------------
        ...
        const Mdst_charged* chk01 = &(*i).child(0).child(1).mdstCharged();
        ...
        double dr01,dz01;
        GetImpactParameters(chk01,&dr01,&dz01,2);
        b_tpl->column("dr01", float(dr01));
        b_tpl->column("dz01", float(dz01));

        double cos_polar_01;
        Vector3 ch01_momentum((*i).child(0).child(1).px(), (*i).child(0).child(1).py(), (*i).child(0).child(1).pz());
...
//------------------- information of pi from B (*i).child(1) -------------------------
...
//------------------- B vertex fit with ExKfitter  -------------------------
        ...
        Mdst_charged Mdst_00 =(*i).child(0).child(0).mdstCharged();
        Mdst_charged Mdst_01 =(*i).child(0).child(1).mdstCharged();
	Mdst_charged Mdst_1 =(*i).child(1).mdstCharged();

        ExKFitterParticle KF_00(Mdst_00, 3);
        ExKFitterParticle KF_01(Mdst_01, 2);
	ExKFitterParticle KF_1(Mdst_1, 2);

        HepPoint3D D0_init;

        D0_init.setX(IP.x() + (*i).child(0).p().px()/(*i).child(0).p().rho());
        D0_init.setY(IP.y() + (*i).child(0).p().py()/(*i).child(0).p().rho());
        D0_init.setZ(IP.z() + (*i).child(0).p().pz()/(*i).child(0).p().rho());
        ExKFitterVertex D0_Vertex(D0_init);
	ExKFitterVertex B_Vertex(IP,IPerr);

        ExKFitterParticle D0;
        D0.LinkParticle(&KF_00);
        D0.LinkParticle(&KF_01);
        D0.LinkVertex(&D0_Vertex);
        ExKFitterConstrain con1;
        con1.SetVertexConstrain();
        con1.LinkParticle(&KF_00);
        con1.LinkParticle(&KF_01);
        con1.LinkVertex(&D0_Vertex);
	...
	Core.LinkConstrain(&con2);
        ...
        HepPoint3D dvertex = D0_Vertex.Vertex();
...
if (MCstatus == 1)
{
        const Mdst_charged ch_00 = (*i).child(0).child(0).mdstCharged();
        const Mdst_charged ch_01 = (*i).child(0).child(1).mdstCharged();
	...
	b_tpl->column("hi1", Evtch_1.idhep());
        ...
        if (Evtch_00.mo(0))
        {
            EvtP_00=gen_mgr[Evtch_00.mo(0)-1];
        ...
        if(Evtch_00.idhep() == -321 && Evtch_01.idhep() == 211 && EvtP_00.idhep() == 421 && EvtP_01.idhep() == 421 && 
	Evtch_1.idhep() == -211 && EvtP_1.idhep() == -521 && EvtGP_00.idhep() == -521 && EvtGP_01.idhep() == -521)
        { hindex = 1;}
        else if (Evtch_00.idhep() == 321 && Evtch_01.idhep() == -211 && EvtP_00.idhep() == -421 && EvtP_01.idhep() == -421 && 
        Evtch_1.idhep() == 211 && EvtP_1.idhep() == 521 && EvtGP_00.idhep() == 521 && EvtGP_01.idhep() == 521)
        { hindex = 1;}
        else
        { hindex = 0;}
...
//void ana_d0pi_20141230::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P)
void ana_d0pi_20141230::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13],Vector4 &otherB_P, HepPoint3D &bvertex, int & vertexflag, HepPoint3D & overtex)
{
...
void ana_d0pi_20141230::GetImpactParameters(const Mdst_charged *charged, double *dr, double *dz, int t) 
     {
