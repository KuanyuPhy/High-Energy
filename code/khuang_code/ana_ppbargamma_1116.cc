...
// Module class
class ana_ppbargamma_1116 : public Module 
{
public:
  ana_ppbargamma_1116(void){};
  ~ana_ppbargamma_1116(void){};
  void init(int*);
  void term(void);
...
extern "C" Module_descr *mdcl_ana_ppbargamma_1116()
{
  ana_ppbargamma_1116 *module = new ana_ppbargamma_1116;
  Module_descr *dscr = new Module_descr ( "ana_ppbargamma_1116", module );
...
void 
ana_ppbargamma_1116::hist_def(void)
{ 
...
//branch name length--> 8 characters(upper limit)
  b_tpl = BASF_Histogram->ntuple("Btoppbarg","mbc modmbc de ebeam EVTID RUNID EXPID eventid farmid \
  pxb pyb pzb eb \
  massp1 pxp1 pyp1 pzp1 ep1 pidp1kp pidp1ppi pidp1pk chrgp1 drp1 dzp1 cosp1 \
  massp2 pxp2 pyp2 pzp2 ep2 pidp2kp pidp2ppi pidp2pk chrgp2 drp2 dzp2 cosp2 \
  pxGam1  pyGam1  pzGam1  epGam1 pi0veto1 pi0prob1 \
  m_ppb chisqexk \
  hindex hi2 hi0 hi1 mo2 mo0 mo1 gmo0 gmo2 gmo1 \
...
void ana_ppbargamma_1116::init(int*)
{
  evtcount=0;
...
void ana_ppbargamma_1116::term (void)
{
...
void ana_ppbargamma_1116::begin_run(BelleEvent *evptr, int *status)
{
...
void 
ana_ppbargamma_1116::event(BelleEvent *evptr, int *status)
{ 
  ...
  combination(B_cand1,ptype_B0,p_plus,p_minus,gamma);
  combination(B_cand1,ptype_antiB0,p_plus,p_minus,gamma);
...
//------------------- information of p- (p2) (*i).child(1) -----------------------
      b_tpl->column("massp2", (*i).child(1).p().mag());
      b_tpl->column("pxp2", (*i).child(1).px());
      b_tpl->column("pyp2", (*i).child(1).py());
      b_tpl->column("pzp2", (*i).child(1).pz());
      b_tpl->column("ep2", (*i).child(1).e());

      //boosted to CMS frame
      HepLorentzVector p2_p((*i).child(1).p() );
      p2_p.boost( boost_vector.boostVector() );

      
      const Mdst_charged* chp2 = &(*i).child(1).mdstCharged();
      float kpi_idp2_kp = selkp.prob(chp2);
      b_tpl->column("pidp2kp", kpi_idp2_kp);
      
      float kpi_idp2_ppi = selppi.prob(chp2);
      b_tpl->column("pidp2ppi", kpi_idp2_ppi);
      
      float kpi_idp2_pk = selpk.prob(chp2);
      b_tpl->column("pidp2pk", kpi_idp2_pk);

      
      float chrgp2 = chp2->charge();
      b_tpl->column("chrgp2", chrgp2);
      
      double drp2,dzp2;
      GetImpactParameters(chp2,&drp2,&dzp2,3);
      b_tpl->column("drp2", float(drp2));
      b_tpl->column("dzp2", float(dzp2));
      
      double cos_polar_p2;
      Vector3 chp2_momentum((*i).child(1).px(), (*i).child(1).py(), (*i).child(1).pz());
      ...
      cos_polar_p2=cos_polar_p2/(chp2_momentum.mag()*P_BEAM_polarp1.mag());
      b_tpl->column("cosp2", cos_polar_p2);
      ...
      b_tpl->column("m_ppb",p_dib.mag()); // Save the magnitude of the 4 monentum -->mass 
	...
	if(tmp.e()>0.03){
      ...
      double E_diff, one_epGam1, one_epGam2, scpx, scpy, scpz, sce;
      //std::cout<<"Modyfied Mbc"<<std::endl;
      E_diff = ebeam - p1_p.e() - p2_p.e();
      one_epGam1 = E_diff/(Gam1_p.e());

      scpx= Gam1_p.px()*one_epGam1;
      scpy= Gam1_p.py()*one_epGam1;
      scpz= Gam1_p.pz()*one_epGam1;
      sce = Gam1_p.e()*one_epGam1;
      Gam1_p.setPx(scpx);
      Gam1_p.setPy(scpy);
      Gam1_p.setPz(scpz);
      Gam1_p.setE(sce);
      ...
      HepLorentzVector b_pMody(p1_p +p2_p+ TGam_p);
      ...
      Mdst_charged Mdst_1   =(*i).child(1).mdstCharged(); //proton
      
      ExKFitterParticle KF_0(Mdst_0, 4); //proton
      ExKFitterParticle KF_1(Mdst_1, 4); //proton
	...
	const Mdst_charged ch_1   =(*i).child(1).mdstCharged(); //proton
	...
	//B^{+} decay, Lambda-bar decay to p+ (100) and pi- (101)
        if( Evtch_0.idhep()== 2212 && EvtP_0.idhep()== 510 && EvtGP_0.idhep()== 511 && Evtch_1.idhep()== -2212 && EvtP_1.idhep()== 510 && EvtGP_1.idhep()== 511 ){
	  //list all possible situations for gamma ray id
	  if (  mo2==511 || gmo2==511 || ggmo2==511
		){ hindex = 1;}
	}
	//B^{-} decay, Lambda decya to p-(101) and pi+ (100)
        else if( Evtch_0.idhep()== 2212 && EvtP_0.idhep()== 510 && EvtGP_0.idhep()== -511 && Evtch_1.idhep()== -2212 && EvtP_1.idhep()== 510 && EvtGP_1.idhep()== -511 ){
	  if( mo2== -511 || gmo2== -511 || ggmo2== -511
	     ){ hindex = 1;}
	}
        else
...
//void ana_ppbargamma_1116::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13], Vector4 &otherB_P)
void ana_ppbargamma_1116::shape(Particle &b, float &R2, float &spher, float &cos_thr, float &cos_thp, float par_sfw[13],Vector4 &otherB_P, HepPoint3D &bvertex, int & vertexflag, HepPoint3D & overtex)
{
...
void ana_ppbargamma_1116::GetImpactParameters(const Mdst_charged *charged, double *dr, double *dz, int t) 
     {
